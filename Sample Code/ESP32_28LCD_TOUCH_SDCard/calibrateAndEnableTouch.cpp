/* =====================================================================
 *  Touch position calibration, module version (called from ESP32_28LCD_TOUCH_SDCard.ino)
 *
 *  Differences from the original stand-alone sketch:
 *    1. setup() / loop() removed        -- supplied by the .ino
 *    2. lcdScreen / touchScreen removed -- defined in the .ino, referenced here with extern
 *    3. All pin and resolution #define removed -- taken from config.h instead
 *    4. init() / setRotation() / invertDisplay() are no longer called here;
 *       the .ino initialises the display, this module only draws on it
 *    5. runCalibration() loops instead of recursing, so repeated failures cannot overflow the stack
 *    6. A calValid flag was added, so getTouch() emits nothing while uncalibrated
 *    7. Waiting for a touch does not time out by default (TAP_TIMEOUT_MS = 0); it waits for the user
 * ===================================================================== */
...
// ===================== Module parameters =====================
#define Z_MIN            400      // minimum pressure
#define SAMPLES           16      // samples taken per target
#define NPT                5      // number of targets
#define CAL_RMS_MAX     6.0f      // residual limit; above this the run is repeated
#define TAP_TIMEOUT_MS     0      // per-step wait for a touch, in ms. 0 = no timeout, wait indefinitely
                                  // note: with 0, a faulty or misdetected touch chip blocks calibration forever
#define MAX_ATTEMPT        3      // total number of attempts

// ===================== Calibration coefficients =====================
struct Cal {
  float    a, b, c, d, e, f;
  uint8_t  rotation;             // screen rotation at save time; the coefficients expire if it changes
  uint32_t magic;
};
...
// Drawing coordinate system, follows the current rotation. At rotation 2 that is the 240x320 from config.h
...
// =====================================================================
//  Sampling: always take 16 readings, sort them and average the middle half
// =====================================================================
...
// Wait for one press. With TAP_TIMEOUT_MS = 0 this never returns false, it keeps waiting
...
// =====================================================================
//  3x3 system of equations (Cramer's rule)
// =====================================================================
...
bool loadCalibration() {
  ...
  // Screen rotation changed, the old coefficients cannot be used
  if (ok && cal.rotation != lcdScreen.getRotation()) {
    Serial.printf("NVS coefficients are for rotation %d, current rotation is %d, recalibration needed\n",
                  cal.rotation, lcdScreen.getRotation());
    ok = false;
  }

  calValid = ok;
  if (ok) Serial.println("Touch calibration coefficients loaded from NVS");
  return ok;
}
...
void clearCalibration() {
  ...
  prefs.end();
  calValid = false;
  Serial.println("Touch calibration coefficients cleared from NVS");
}
...
bool getTouch(int16_t &x, int16_t &y) {
  if (!calValid) return false;                 // not calibrated, emit no coordinates
  if (!touchScreen.touched()) return false;
  TS_Point p = touchScreen.getPoint();
...
// =====================================================================
//  Print a snippet ready to paste
// =====================================================================
static void printSnippet(float rms) {
  Serial.println();
  Serial.println("========= Copy the block below into your sketch =========");
...
// =====================================================================
//  Main calibration flow (retries in a loop, no longer recursive)
// =====================================================================
void runCalibration() {
  const int16_t W = scrW(), H = scrH();

  // 5 targets: four corners at 15% / 85% plus the centre
  ...
  calValid = false;
  Serial.printf("===== XPT2046 touch calibration  drawing area %d x %d (rotation %d) =====\n",
  ...
  for (int attempt = 1; attempt <= MAX_ATTEMPT; attempt++) {
    ...
    centerText("Tutorial,Code URL:",                H/2 + 100, ST77XX_YELLOW,   2);
    centerText(" www.github.com/GJKJ/ESP3228LCD",   H/2 + 120, ST77XX_WHITE,   2);

    waitRelease();
    if (!waitForTap()) {
      Serial.println("Timed out waiting for a touch, calibration abandoned");
      lcdScreen.fillScreen(ST77XX_BLACK);
    ...
    if (timedOut) {
      Serial.println("Timed out while sampling, calibration abandoned");
      lcdScreen.fillScreen(ST77XX_BLACK);
      centerText("CAL TIMEOUT", H/2 - 10, ST77XX_RED, 2);
      centerText("send 'c' to retry", H/2 + 15, ST77XX_WHITE, 1);
      return;
    ...
    if (!fitAffine(tgtX, tgtY)) {
      ...
      Serial.println("Fit failed: target data is degenerate, retrying");
      delay(2000);
      continue;
    }
    ...
    if (rms > CAL_RMS_MAX) {
      ...
      Serial.println("Residual too large, retrying");
      delay(2500);
      continue;
    }
  ...
  Serial.println("Calibration failed three times, staying uncalibrated");
  lcdScreen.fillScreen(ST77XX_BLACK);
