...
void setup() {
  ...
  if (!SD.begin(SD_CS)) {
    ...
    if (!SD_file) {
      lcdScreen.setTextColor(ST77XX_RED);
      lcdScreen.println("Write File...Fail!");
      return;
    } else {
      const char *msg = "Source Code,Tutorial,Pin Reference Download URL: www.github.com/GJKJ/ESP3228LCD";
      size_t n = SD_file.println(msg);
      SD_file.close();
      bool sdWriteOK = (n >= strlen(msg));
      lcdScreen.setTextColor(sdWriteOK ? ST77XX_WHITE : ST77XX_RED);
      lcdScreen.println(sdWriteOK ? "Write File...OK!" : "Write File...Fail!");
    }
  }
