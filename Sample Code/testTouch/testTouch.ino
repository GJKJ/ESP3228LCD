...
// Raw touch range. Run this once, read the raw values printed on the serial monitor and edit these four.
// Swap the MIN and MAX values to reverse an axis.
...
void setup() {
  ...
  pinMode(13, OUTPUT); digitalWrite(13, HIGH);   // SD not used here; hold its CS HIGH to free the SPI bus
...
void loop() {
  if (touchScreen.touched()) {
    ...
    int16_t rx = p.x, ry = p.y;                  // swap p.x and p.y if the axes come out the wrong way round
